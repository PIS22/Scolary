...
@Component({
  ...
  styleUrl: './tf-form.component.scss'
})
export class TfFormComponent  implements OnInit {
  ...
  submit() {
    let body = {
      ...
      codeTypeFrais: this.dataForm.value.code,
      libTypeFrais: this.dataForm.value.libe,
    };
    if (body.id) {
      this.service.editTypeFrais(body).subscribe(
        (res) => {
      ...
      this.service.createTypeFrais(body).subscribe(
        (res) => {
          console.log(res);
  ...
  initForm() {
    this.dataForm = this.fb.group({
      ...
      code: [this.valueIn ? this.valueIn.codeTypeFrais : null, Validators.required],
      libe: [this.valueIn ? this.valueIn.libTypeFrais : null, Validators.required],
    });
  }
